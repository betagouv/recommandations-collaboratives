...
test.describe(
  'I can delete a link on the document tab',
  ...
  () => {
    test('deletes a link', async ({ page }) => {
      ...
      await page
        .getByText(documents[0].fields.description)
        .first()
        .locator('xpath=../../../..')
        .locator('#link-delete-button')
        .dispatchEvent('click');
    ...
    test('must not show the deleted link', async ({ page }) => {
      ...
      await expect(
        page.getByText(documents[0].fields.description)
      ).toHaveCount(0);
    });
  }
);
