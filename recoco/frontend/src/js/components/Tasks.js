...
export default function TasksApp(app) {
  const taskApp = {
    ...
    get view() {
      const result = this.$store.tasksView.displayedTasks
    ...
    loadCanAdministrate() {
      const canAdministrate =
        document.getElementById('canAdministrate').textContent;
      this.canAdministrate = JSON.parse(canAdministrate);
    },
    loadUserProjectPerms() {
      const userProjectPerms =
        document.getElementById('userProjectPerms').textContent;
      this.$store.tasksData.canUseTasks =
        userProjectPerms.indexOf('use_tasks') > -1;
      this.$store.tasksData.canManageTasks =
        userProjectPerms.indexOf('manage_tasks') > -1;
    },
    loadIsSwitchtender() {
      const isSwitchtender =
