...
function MapEditor(projectOptions) {
  return {
    ...
    async initMap(project, geoData) {
      ...
      const geocoderBAN = mapUtils.initMapControllerBAN(
        Map,
        geoData,
        onClick,
        project,
      ...
      Map.on('click', function (e) {
        ...
        const marker = L.marker(e.latlng, {
          icon: mapUtils.createMarkerIcon('marker-onclick'),
        }).addTo(Map);
        marker.bindPopup(
          mapUtils.markerPopupTemplate({
            ...popupOptions,
            location_x: e.latlng.lng,
            location_y: e.latlng.lat,
          })
        );
