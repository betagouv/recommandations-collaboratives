...
L.Control.GeocoderBAN = L.Control.extend({
  ...
  onAdd: function (map) {
    ...
    const icon = (this.icon = L.DomUtil.create(
      'span',
      className + '-icon',
      container
    ));
    const form = (this.form = L.DomUtil.create(
      'div',
      className + '-form',
      container
    ));
  ...
  moveSelection: function (direction) {
    ...
    if (!s.length) {
      ...
      if (direction > 0) {
        el = currentSelection.previousElementSibling
          ? currentSelection.previousElementSibling
          : this.alts['lastChild'];
      } else {
        el = currentSelection.nextElementSibling
          ? currentSelection.nextElementSibling
          : this.alts['firstChild'];
      }
    }
    if (el) {
      L.DomUtil.addClass(el, 'leaflet-control-geocoder-ban-selected');
    ...
    switch (e.keyCode) {
      ...
      default:
        if (this.input.value && this.input.value.length > 3) {
          const params = {
            q: this.input.value,
            limit: this.options.resultsNumber,
          };
  ...
  displayResults: function (t) {
    ...
    return function (res) {
      if (res && res.features) {
        ...
        for (
          let i = 0;
          i < Math.min(features.length, t.options.resultsNumber);
          i++
        ) {
  ...
  markGeocode: function (feature) {
    if (this._map) {
      ...
      const marker = new L.Marker(latlng, { icon: this.options.markerIcon })
        .bindPopup(
          this.options.markerPopupTemplate({
            ...this.options.popupOptions,
            address: this.input.value,
          })
        )
        .openPopup();
      this.geocodeMarker = L.layerGroup([marker]).addTo(map);
      this.options.markers[0] = this.geocodeMarker;
    }
  },
});
