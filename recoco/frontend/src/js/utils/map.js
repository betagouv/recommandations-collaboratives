...
function getDefaultLatLngForLayers(project, geoData) {
  const longitude = project.location_x
    ...
    : geoData.location.longitude
      ? geoData.location.longitude
      : undefined;
  const latitude = project.location_y
    ? project.location_y
...
function initSatelliteLayer(lat, lng, zoom) {
  if (!geolocUtils.IGN_BBOX.contains(new L.LatLng(lat, lng))) {
    console.warn(
      'Coordinates outside of IGN Tiles range, stopping initialization'
    );
    return null;
  }
...
function addLayerMarkerProjectCoordinates(map, project) {
  ...
  const marker = L.marker(coordinates, {
    icon: createMarkerIcon('project-coordinates-marker'),
  }).addTo(map);
...
function addLayerMarkerProjectLocation(map, project, geoData) {
  ...
  const popupOptions = { ...project, title: project.name };
  const coordinates =
    geoData.location.features[0].geometry.coordinates.reverse();
  const marker = L.marker(coordinates, {
    icon: createMarkerIcon('project-location-marker'),
  }).addTo(map);
  marker.bindPopup(markerPopupTemplate(popupOptions));
  map.panTo(new L.LatLng(...coordinates));
  return marker;
...
function addLayerParcels(map, geoData) {
  if (
    (geoData.code && geoData.code === 400) ||
    geoData.features?.length === 0
  ) {
    throw Error(
      `Données parcelaires indisponibles pour la commune "${geoData.commune.name}"`
    );
  }
  const parcelLayer = L.geoJSON(
    geoData.parcels,
    mapLayerStyles('area-parcels')
  );
  const overlayMap = {
