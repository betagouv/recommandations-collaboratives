...
@pytest.mark.resource_update
@pytest.mark.django_db
def test_update_resource_from_non_origin_site_with_bad_form(request, client):
    original_resource = baker.make(models.Resource)
    current_site = get_current_site(request)
    other_site = baker.make(Site)
    ...
    bad_data = {
        ...
        "summary": "a summary",
        "tags": "#tag",
        "content": "this is some badly formatted content",
    }
    ...
    assert original_resource.content
...
def test_update_resource_from_non_origin_site_and_redirect(request, client):
    ...
    assert other_site == original_resource.site_origin
