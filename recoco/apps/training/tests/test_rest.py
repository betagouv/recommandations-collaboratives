...
def test_challenge_get_api_returns_existing_challenge_info(request, client):
    ...
    assert datetime.fromisoformat(response.data["started_on"]) == started_on
    assert dict(response.data["challenge_definition"]) == {
        "name": definition.name,
        "code": definition.code,
        "description": definition.description,
...
@pytest.mark.django_db
def test_challenge_get_api_does_not_returns_existing_challenge_info_other_user(
    request, client
):
    site = get_current_site(request)
    user = baker.make(
    ...
    other_user = baker.make(
        auth_models.User, email="other@example.com", last_login=timezone.now()
    )
    definition = baker.make(models.ChallengeDefinition, site=site)
    started_on = timezone.now()
    baker.make(
