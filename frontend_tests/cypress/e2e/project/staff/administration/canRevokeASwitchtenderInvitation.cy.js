...
describe('I can go to administration area of a project and revoke an invite for a switchtender', () => {
    beforeEach(() => {
        cy.login("staff");
    })
    ...
    it('goes to the administration tab of a project and revoke the switchtender invitation', () => {
        ...
        cy.get("[data-test-id='administration-advisor-invitation-list']").siblings('ul').children('li').contains(userToInvite.fields.email).parent().parent().parent().siblings().find('#revoke-invite-switchtender').click({force:true})
