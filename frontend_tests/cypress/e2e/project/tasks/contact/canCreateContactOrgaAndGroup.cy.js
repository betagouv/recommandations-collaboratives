describe('I can create and edit a contact and an organization on contactbook', () => {
  ...
  xit('can create a contact and create a new organization with no group and no departments', () => {
    ...
    cy.get('[data-test-id="button-create-organization"]').click({ force: true });
    ...
    cy.get('[data-test-id="contact-card"]').should('contain', 'Anakin Skywalker')
      .and('contain', 'jedicorp');
  });
  ...
  xit('can create a contact and search an existing organization', () => {
    ...
    cy.get('[data-test-id="orga-to-select"]').find('span').contains('jedicorp').click();
    //add job title
    cy.get('[data-test-id="job"]').type('Jedi Knight');
    //submit the contact form
    cy.get('[data-test-id="create-contact-button"]').click({ force: true });
  ...
  xit('can create a contact and create a new organization with an existing group and no departments', () => {
    cy.visit(`/addressbook/contacts`);
    //click on create contact button
    cy.get('[data-test-id="button-create-contact"]').click({ force: true });
    //fill in the contact form
    ...
    cy.get('[data-test-id="orga-group-to-select"]').find('span').contains('Jedicorp').click();
    //create the organization
    cy.get('[data-test-id="button-create-new-organization"]').click();
    //add job title
  ...
  it('can create a contact and create a new organization and create a group and select departments', () => {
    ...
    cy.get('[data-test-id="select-list-options"]').find('div').contains('(93) Département de test numéro 3').click();
    //select yes for national group
    cy.get('#natGroup-yes').click({ force: true });
  ...
  xit('can create a contact and create a new organization with an existing group and one department', () => {
    ...
    //search for a non existing organization
    cy.get('#search-organization-input').type('between master and knight jedi corp');
    //click on create organization button
    cy.get('[data-test-id="button-create-organization"]').click({ force: true });
    //select a department
    cy.get('#select-list-input').click();
    ...
    cy.get('[data-test-id="contact-card"]').should('contain', 'obiwan kenobi')
      .and('contain', 'between master and knight jedi corp');
  });
