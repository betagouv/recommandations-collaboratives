...
describe('I can invite a member', () => {
    ...
    it('goes to the overview page and invite a member', () => {
        ...
        cy.get('#invite-member-modal').contains("Envoyer l'invitation").click({force:true}).wait(500)
    ...
    it('can see a notification of the invitation on the project activity feed', () => {
        ...
        cy.login("staff");
        cy.visit(`/project/${currentProject.pk}`)
        cy.get('[data-test-id="fr-consent-banner"]').find('[data-test-id="button-consent-accept-all"]').click({ force: true }).wait(300)
        cy.get('[data-test-id="project-activity-link"]').click({force:true})
        cy.get('[data-test-id="project-activity-tracking-staff"]').find('[data-test-id="project-activity-notification"]').then(()=>{
