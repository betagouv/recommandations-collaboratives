...
describe('I can go to overview tab', () => {
    ...
    it('show the tooltip of a non active user with the date of last connection', () => {
        cy.get('[data-test-id="project-information-card-context"] [data-test-id="button-open-tooltip-user-card"]')
            .click({ force: true })
            .then(() => {
                cy.get('[data-test-id="project-information-card-context"] [data-test-id="user-card-intro"]')
                    .invoke('text')
                    .should((text) => {
                        expect(text.length).to.be.greaterThan(0)
                    })
                cy.get('[data-test-id="deactivated-user-details"]').should('exist')
        })
    })
    ...
    it('show an anonymous user card if the user does not exist', () => {
        ...
        cy.get('[data-test-id="project-information-card-context"] [data-test-id="button-open-tooltip-user-card"]')
            .click({ force: true })
            .then(() => {
                cy.get('[data-test-id="project-information-card-context"] [data-test-id="user-card-intro"]').should('not.exist')
