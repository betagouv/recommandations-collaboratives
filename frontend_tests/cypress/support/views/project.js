...
class Project {
  ...
  deactivateProject() {
    cy.get(this.dom.ADMIN_BANNER_DEACTIVATE_PROJECT)
      .get(this.dom.BUTTON_MODAL_DEACTIVATE_PROJECT)
      .click({ force: true });
    cy.get(this.dom.FORM_PAUSE_PROJECT)
      .get(this.dom.BUTTON_DEACTIVATE_PROJECT)
      .click({ force: true });
  ...
  activateProjectFromPreferences() {
    cy.get(this.dom.ADMIN_BANNER_ACTIVATE_PROJECT)
  ...
  closeEmailReminderTooltip(condition = 'not.exist') {
    cy.get(this.dom.BUTTON_CLOSE_REMINDER_SETTINGS)
      .click({ force: true })
      .then(() => {
