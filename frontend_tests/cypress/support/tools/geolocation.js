...
const domElements = {
    ...
    BUTTON_SAVE_PROJECT_LOCATION:
        '[data-test-id="button-save-project-location"]',
};
...
class ProjectLocation {
    ...
    openMapModal() {
        ...
        cy.get(this.dom.PROJECT_MAP_INTERACTIVE).find(
            this.dom.LEAFLET_CONTROL_ZOOM
        );
    }
    ...
    editProjectLocationUsingAddressField(address) {
        ...
        cy.get(this.dom.SELECT_ADDRESS_LOCATION_EDIT)
            .click({ force: true })
            .then(() => {
                cy.get(this.dom.SELECT_ADDRESS_LOCATION_EDIT).click({
                    force: true,
                });
            });
    }
    ...
    checkMapLayerProjectCoordinates(map = 'map-static') {
        cy.get(this.getSelector(map))
            ...
            .then(() => {
                cy.get(this.dom.LEAFLET_MARKER_PROJECT_COORDINATES).should(
                    'exist'
                );
    ...
    checkMapLayerAreaCommune(map = 'map-static') {
        cy.get(this.getSelector(map))
            .get(this.dom.LEAFLET_LOCATION_OVERLAY_PANE)
            .then(() => {
    ...
    checkMapLayerCircle(condition = 'exist', map = 'map-static') {
        ...
        cy.get(this.getSelector(map))
            .find(this.dom.LEAFLET_LOCATION_OVERLAY_PANE)
            .then(() => {
